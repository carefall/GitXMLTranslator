    ...
    internal class GitHubClient
    {
        ...
        public static async Task<int> Check(string gameDataPath)
        {
            try
            {
                ...
                foreach (string localFile in localJsons)
                {
                    ...
                    if (hashesDict[localFile] != ComputeFileSha1(gameDataPath + "/gamedata/configs/" + path)) filesToUpdate[localFile] = path;
                }
                ...
                if (filesToUpdate.Count > 0 || filesToDelete.Count > 0)
                {
                    ...
                    foreach (var path in filesToDelete)
                    {
                        ...
                        string text = File.ReadAllText(gameDataPath + "/gamedata/configs/" + path);
                        var dir = AppDomain.CurrentDomain.BaseDirectory + "/Backups";
                        var fileFolders = path.Split('/');
                        var fileFolderWithoutFile = "/" + fileFolders[0];
                        for (int i = 1; i < fileFolders.Length - 1; i++)
                        {
                            fileFolderWithoutFile += "/" + fileFolders[i];
                        }
                        dir += fileFolderWithoutFile;
                        if (!Directory.Exists(dir))
                        {
                            Directory.CreateDirectory(dir);
                        }
                        File.WriteAllText(dir + "/" + fileFolders[fileFolders.Length - 1], text);
                        File.Delete(gameDataPath + "/gamedata/configs/" + path);
                    }
                    return 1;
                }
                return 0;
        ...
        public static async Task DownloadFile(string file, string owner, string repo)
        {
            try
            {
                ...
                var data = await client.GetByteArrayAsync($"https://raw.githubusercontent.com/{owner}/{repo}/main/gamedata/configs/{file}");
